...
Full command list: `readability`, `lint` (+ `lint rules`), `diff`, `metrics list|show`, `entities`, `sentiment`, `classify`, `kb lookup|search`, `fetch`, `research papers|paper|similar`, `config`, `update`.
...
| `fetch`, `--url` | **billed per page** (Firecrawl), cached 24h | `FIRECRAWL_API_KEY` |
